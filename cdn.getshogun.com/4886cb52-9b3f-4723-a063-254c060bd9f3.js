...
;(function(root, factory) {
  ...
  function SHGMoney(platform, config) {
    var defaultConfig = {
      shopify: "${{amount}}", // eslint-disable-line no-template-curly-in-string
      big_commerce: {
        currency: "USD",
        currency_symbol: "$",
        decimal_separator: ".",
...
;(function($) {
  ...
  if ($(document).attr("shg-lightbox") && !$(".shogun-lightbox-item").length) return
  $(document).attr("shg-lightbox", 1)

  var modal = $(
    '<div class="shogun-lightbox">\n' +
      '  <div class="shogun-lightbox-content">\n' +
      '    <span class="shogun-lightbox-close">&times;</span>\n' +
      '    <div class="shogun-lightbox-image-container">\n' +
      '      <img class="shogun-lightbox-image" />\n' +
      "    </div>\n" +
      "  </div>\n" +
      "</div>"
  ).appendTo("body")

  $(document).on("click", ".shogun-lightbox-item", function(e) {
    var img = $(this).prev()
    modal.find("img").attr("src", img.attr("src"))
    modal.show()
  })

  $(document).on("click", ".shogun-lightbox-close", function(e) {
    $(this)
      .closest(".shogun-lightbox")
      .hide()
  })

  window.onclick = function(event) {
    if (
      (event.target === modal[0] || $(modal[0]).has(event.target).length) &&
      !$(event.target).is("img")
    )
      modal.hide()
  }

  window.SHOGUN_BROWSER.objectFitPolyfill()
})(SHGJQ)
...
;(function($) {
  ...
  function adjust() {
    ...
    $viableContainers.each(function(i, container) {
       ...
       */
      var colWrapped = false
      var prevOffset
      $cols.each(function(colIdx, col) {
        if (!prevOffset) {
          prevOffset = $(col).offset().top
        }
        if (prevOffset !== $(col).offset().top) {
          colWrapped = true
        }
      ...
      if (!colWrapped) {
        Object.keys(rows).forEach(function(rowIdx) {
          rows[rowIdx].forEach(function($elem) {
            if (heights[rowIdx]) {
